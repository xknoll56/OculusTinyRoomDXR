...
struct DirectX12
{
    ...
    NVSDK_NGX_Handle* ngxFeatureHandle;



    // Raytracing output
    ComPtr<ID3D12Resource> m_raytracingOutputs[4];
    D3D12_GPU_DESCRIPTOR_HANDLE m_raytracingOutputResourceUAVGpuDescriptors[4];
    UINT m_raytracingOutputResourceUAVDescriptorHeapIndexs[4];
...
// with all configuration options resolved, such as local signatures and other state.
    ...
    bool InitDevice(int vpW, int vpH, const LUID* pLuid, DXGI_FORMAT depthFormat, int eyeMsaaRate, bool windowed = true, UINT eyeWidth=1024, UINT eyeHeight=768)
    {
        ...
        // Initialize dlss
        wchar_t buffer[1000];
        GetModuleFileName(NULL, (LPWSTR)buffer, 1000);
        NVSDK_NGX_Result result = NVSDK_NGX_D3D12_Init(0, buffer, Device);
        ThrowIfFalse(result == NVSDK_NGX_Result_Success);
        ...
        // Set any necessary parameters, for example:
        ngxParams->Set(NVSDK_NGX_Parameter_Scale, 2.0f); // For 50% scaling
...
// This encapsulates all shader records - shaders and the arguments for their local root signatures.
    ...
    void CreateRaytracingOutputResource(UINT width, UINT height)
    {
        ...
        for(int i = 0; i<4; i++)
        {
            if (i == 2)
            {
                //create the super sampled back buffers
                uavDesc = CD3DX12_RESOURCE_DESC::Tex2D(backbufferFormat, width*2, height*2, 1, 1, 1, 0, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
            }
            ThrowIfFailed(Device->CreateCommittedResource(
                &defaultHeapProperties, D3D12_HEAP_FLAG_NONE, &uavDesc, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(&m_raytracingOutputs[i])));
            //NAME_D3D12_OBJECT(m_raytracingOutput);
            D3D12_CPU_DESCRIPTOR_HANDLE uavDescriptorHandle = CbvSrvHandleProvider.AllocCpuHandle(&m_raytracingOutputResourceUAVDescriptorHeapIndexs[i]);
            D3D12_UNORDERED_ACCESS_VIEW_DESC UAVDesc = {};
            UAVDesc.ViewDimension = D3D12_UAV_DIMENSION_TEXTURE2D;
            Device->CreateUnorderedAccessView(m_raytracingOutputs[i].Get(), nullptr, &UAVDesc, uavDescriptorHandle);
            m_raytracingOutputResourceUAVGpuDescriptors[i] = CbvSrvHandleProvider.GpuHandleFromCpuHandle(uavDescriptorHandle);
        }
    ...
    void CopyRaytracingOutputToBackbuffer(ID3D12Resource* renderTarget, ID3D12Resource* depthTarget)
    {
        ...
        ngxParams->Set(NVSDK_NGX_Parameter_Depth, m_raytracingDepthOutputs[ActiveContext].Get());
        NVSDK_NGX_Handle inBufferHandle, outBufferHandle;
        ngxParams->Set(NVSDK_NGX_Parameter_Input1, m_raytracingOutputs[ActiveContext].Get());
        ngxParams->Set(NVSDK_NGX_Parameter_Output, m_raytracingOutputs[ActiveContext + 2].Get());
        NVSDK_NGX_Result result = NVSDK_NGX_D3D12_EvaluateFeature(currFrameRes.CommandLists[ActiveContext], ngxFeatureHandle, ngxParams);
        ...
        preCopyBarriers[2] = CD3DX12_RESOURCE_BARRIER::Transition(m_raytracingOutputs[ActiveContext+2].Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_COPY_SOURCE);
        ...
        currFrameRes.CommandLists[ActiveContext]->CopyResource(renderTarget, m_raytracingOutputs[ActiveContext+2].Get());
        ...
        postCopyBarriers[2] = CD3DX12_RESOURCE_BARRIER::Transition(m_raytracingOutputs[ActiveContext+2].Get(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
